...
function BusCard({ service }) {
  ...
  return (
    <div className={styles.card}>
      <img
        src={returnOperatorLogoLink(service.Operator)}
        className={styles.cardImgTop}
        alt={service.Operator}
      />
      <div className={styles.cardBody}>
        <h5 className={styles.cardTitle}>Bus {service.ServiceNo}</h5>
        <p className={styles.cardText}>
          Arriving in{" "}
          {calculateMinutesTillArrival(service.NextBus.EstimatedArrival)} mins
        </p>
      </div>
    </div>
  );
